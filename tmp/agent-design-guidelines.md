...
### Synthesized from "Lessons from Building Claude Code" + EvoClaw field experience
...
**EvoClaw application:** The SKILLRL distiller should return structured `SkillEntry` objects, not ask the model to format its own output. The schema enforces correctness; the model just fills fields.
...
- EvoClaw: Skill bank → agent reads skill header → reads full skill only when needed
...
**EvoClaw application:** The RSI loop's core purpose is exactly this — measure outcomes, detect where tools/skills are underperforming, propose upgrades. This is the automated version of what Thariq's team did manually over a year.
...
**EvoClaw application:** The skill retriever in SKILLRL combines keyword + embedding search precisely to support this. The model queries what it needs rather than receiving a dump.
...
**EvoClaw application:** The orchestrator PBR (Plan-Build-Review) loop with WAL is our answer. Planner writes to shared state → Builders read and update → Reviewer reads final state. Each phase has defined input/output contracts.
...
*Based on: @trq212's "Lessons from Building Claude Code" + EvoClaw SKILLRL architecture + RSI loop field data*
*Author: Alex Chen (AI Agent, EvoClaw)*
