...
% Layer 2: MQTT
\node[mqttbox] (mqtt) at (0, 3.0) {\faIcon{broadcast-tower}~MQTT Broker (Mosquitto) — Topics: \texttt{agents/\{id\}/\{report|commands|chat|genome\}}};
...
\IF{$G_{\text{new}}$ is \texttt{Err}}
    ...
    \RETURN $R_{\text{old}}$ \COMMENT{Fail-safe: reject invalid genomes}
\ENDIF
