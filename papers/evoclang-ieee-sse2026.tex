...
The rollback mechanism ensures monotonic improvement in expectation: at each iteration, either fitness improves (mutation retained) or the genome is restored (rollback triggered when fitness decrease exceeds $\delta$). By construction, expected fitness $\mathbb{E}[\fitnessfn(g_t)]$ is non-decreasing over evolution iterations $t$.
