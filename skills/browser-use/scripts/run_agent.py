...
def gemini_llm(model: str = "gemini-2.5-flash"):
    """
    Return an LLM for browser-use agent.
    Uses Cerebras (free, fast, OpenAI-compatible) as primary.
    Falls back to Gemini Cloud Code Assist if needed.
    """
    from langchain_openai import ChatOpenAI

    # Cerebras — free, fast, OpenAI-compatible
    cerebras_key = "csk-4234tt48dhftvcynnj3t6hxtkfjc59ehj25f22nfhmhnxjwy"
    return ChatOpenAI(
...
def stealth_session(
    ...
    """
    Return a Browser instance hardened against bot detection.
    Updated for browser_use 0.2+ API (Browser + BrowserConfig).
    """
...
async def main(task: str, model: str, headless: bool, output_json: bool):
    ...
    browser = stealth_session(headless=headless)
